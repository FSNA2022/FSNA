...
from .gdl_last import decouple_layer, AffineLayer
import ipdb
...
class StandardROIHeads(ROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        """
        # ipdb.set_trace()
        # box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])  # torch.Size([4096, 1024])
        scale_proposals = proposals
        scale_boxes = []
        for index, i in enumerate([x.proposal_boxes for x in scale_proposals]):
            h, w = int(str(scale_proposals[index]).split(',')[1].split('=')[1]), int(
                str(scale_proposals[index]).split(',')[2].split('=')[1])
            boxes = i.tensor.cpu().numpy().tolist()
            for box in boxes:
                width = box[2] - box[0]
                height = box[3] - box[1]
                d_w = 0.4 * width / 2
                ...
                if box[3] > h:
                    box[3] = h
            scale_boxes.append(Boxes(torch.tensor(boxes).cuda()))
        scale_box_features = self.box_pooler(features, scale_boxes)
        box_features = self.box_head(scale_box_features)  # [None, FC_DIM] torch.Size([4096, 1024]) #改为scal_box_feature
        pred_class_logits, pred_proposal_deltas = self.box_predictor(box_features)
        # log_feature, cls_feature = self.box_head(box_features)  # [None, FC_DIM] torch.Size([4096, 1024])
        # pred_class_logits, pred_proposal_deltas = self.box_predictor(cls_feature)
        # _, pred_proposal_deltas=self.box_predictor(log_feature)
        del box_features
        outputs = FastRCNNOutputs(
            self.box2box_transform,
            pred_class_logits,
...
class ContrastiveROIHeads(StandardROIHeads):
    ...
    def _forward_box(self, features, proposals):
        ...
        for index, i in enumerate([x.proposal_boxes for x in scale_proposals]):
            ...
            scale_boxes.append(Boxes(torch.tensor(boxes).cuda()))
        # box_features = self.box_pooler(features, [x.proposal_boxes for x in proposals])
        box_features = self.box_pooler(features, scale_boxes)
        # print(box_features)
        # _, box_features = self.box_head(box_features)  # [None, FC_DIM]
        ...
        # pred_class_logits, pred_proposal_deltas = self.box_predictor(cls_feature)
        # _, pred_proposal_deltas = self.box_predictor(log_feature)
        # box_features_contrast = self.encoder(cls_feature)
        del box_features
