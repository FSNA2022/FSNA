...
from .Attn import ALL_Attention,All_att
...
class GeneralizedRCNN(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        # ipdb.set_trace()
        # print("aaaaaaaaaaaaa:",self._SHAPE_)
        self.affine_rpn = AffineLayer(num_channels=self._SHAPE_['p4'].channels, bias=True)
        self.affine_rcnn = AffineLayer(num_channels=self._SHAPE_['p4'].channels, bias=True)
        if self.moco:
    ...
    def forward(self, batched_inputs):
        ...
        # backbone FPN
        # ipdb.set_trace()
        images = self.preprocess_image(batched_inputs)   #Normalize, pad and batch the input images.
        ...
        if self.proposal_generator:
            proposals, proposal_losses = self.proposal_generator(images, features, gt_instances)      #baseline模型的话，改成features
        ...
        _, detector_losses = self.roi_heads(images, features, proposals, gt_instances)          #baseline模型的话，改成features
